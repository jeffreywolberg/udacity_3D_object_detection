...
def bev_from_pcl(lidar_pcl, configs):
    ...
    m_per_px = (configs.lim_x[1] - configs.lim_x[0]) / configs.bev_height

    ## step 2 : create a copy of the lidar pcl and transform all metrix x-coordinates into bev-image coordinates  
    lidar_pcl_cpy = np.copy(lidar_pcl)  
    lidar_pcl_cpy[:, :2] /= m_per_px

    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur

    lidar_pcl_cpy[:, 1] += (configs.bev_width / 2)
    ...
    # My comment: In the instructions it says to sort by Z but in the solution code it sorts by height intensity
    lidar_pcl_cpy[lidar_pcl_cpy[:,3]>1.0,3] = 1.0
    inds = np.lexsort((-lidar_pcl_cpy[:, 3], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
    lidar_pcl_cpy = lidar_pcl_cpy[inds]

    ## step 3 : extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    ##          also, store the number of points per x,y-cell in a variable named "counts" for use in the next task

    vals, inds, counts = np.unique(lidar_pcl_cpy[:, :2], axis=0, return_index=True, return_counts=True)
    ...
    # intensity_map[inds[:, 0], inds[:, 1]] = lidar_pcl_top[:, 3] / (np.amax(lidar_pcl_top[:, 3])-np.amin(lidar_pcl_top[:, 3]))
    ...
    inds = np.lexsort((-lidar_pcl_cpy[:, 2], lidar_pcl_cpy[:, 1], lidar_pcl_cpy[:, 0]))
    
    lidar_pcl_cpy = lidar_pcl_cpy[inds]
    _, inds = np.unique(lidar_pcl_top[:, 0:2], axis=0, return_index=True)
